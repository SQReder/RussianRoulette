...
TBot* bot[5];
void SetQuestionsMaximum(int FirstRound, int SecondRound, int ThirdRound,
	int FouthRound);
// -----------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
void TF::CreateLabel(TLabel** type, int index, int top, int left, int width,
	int height, UnicodeString caption) {
	if (!type[index]) {
...
void __fastcall TF::btnExitClick(TObject* Sender) {
	int r;
	r = MessageDlg
		("Вы действительно хотите закончить игру?\nВсе данные об этой игре будут потеряны!",
		mtCustom, mbYesNo, 0);
	if (r == mrYes) {
		F->Close();
	}
...
void __fastcall TF::tmrRotatorTimer(TObject* Sender) {
	...
	if (MechanizmState == Stoping) // если механизм в стадии отключения
	{
		...
		tmrRotator->Interval = SpeedOfRotation;
		if ((SpeedOfRotation > 450) && (CurrentHatch != 0) &&
			(ingame[CurrentHatch - 1]))
		...
		if (Wait == 3) {
			...
			if (RoundOfGame != 4) {
				if (Settings->PlayerType[CurrentHatch - 1] == bbHuman) {
					...
					chooseplayer = bot[CurrentHatch - 1]->ChooseAnyPlayer
						(ingame, CurrentHatch);
					choosingplayer();
					MechanizmSetHatchesStates();
					ModeOfGame = 2;
...
void __fastcall TF::HatchClick(TObject* Sender) {
	...
	if (FinalRoundOfGame > 0 && CanChoose == 1) {
		...
		for (int i = 0; i < 5; i++) {
			if (ingame[i]) {
				imgPlayer[i]->Top =
					int(imgHatch[id]->Top + (imgHatch[id]->Height -
					imgNumber[0]->Height) / 2 - 5);
				imgPlayer[i]->Left =
					int(imgHatch[id]->Left + (imgHatch[id]->Width -
					imgNumber[0]->Width) / 2 - 5);
			}
		}
	}
...
void __fastcall TF::FormKeyDown(TObject* Sender, WORD& Key, TShiftState Shift) {
	...
	if (FinalRoundOfGame > 0 && ModeOfGame == 0 && Settings->PlayerType
	...
	if (ModeOfGame == 1 && Settings->PlayerType[CurrentHatch -
		1] == bbHuman && CanChoose == 1) {
		for (int i = 1; i < 6; ++i) {
			if (strchr(KeyCode[i],
				Key) && ingame[i - 1] == true && CurrentHatch != i) {
				HatchClick(imgHatch[i]);
			}
		}
	}
	if (ModeOfGame == 3 && CanAnswer == 1 && Settings->PlayerType[chooseplayer -
		1] == bbHuman) {
		for (int i = 1; i < 6; ++i) {
			if (strchr(KeyCode[i],
				Key) && ingame[i - 1] == true && CurrentHatch != i) {
...
void __fastcall TF::tmrWaitingFinalTimer(TObject* Sender) {
			...
			if (Wait == 11) {
				...
				for (int i = 0; i < 5; i++) {
					if (ingame[i]) {
						imgPlayer[i]->Top =
							int(imgPlace->Top +
							(imgPlace->Height - imgPlayer[i]->Height) / 2);
						imgPlayer[i]->Left =
							int(imgPlace->Left +
							(imgPlace->Width - imgPlayer[i]->Width) / 2);
					}
				}
			}
			if (Wait == 18) {
	...
	case 1: {
			...
			if (Wait == 8) {
				...
				imgTimer->Picture->Assign(gfx->Tick[TimeOfQuestion]);
				imgTimer->Visible = True;
				edFinalAnswer->Top = imgQuestion->Top + imgQuestion->Height -
					30 - edFinalAnswer->Height;
				edFinalAnswer->Left =
					(int)(imgQuestion->Left +
					(imgQuestion->Width - edFinalAnswer->Width) / 2.);
	...
	case 3: { // игрок даёт ответ
			...
			if (_Parse(edFinalAnswer->Text)) {
				...
				lblRightAnswer->Left =
					(int)(imgQuestion->Left + (imgQuestion->Width / 2.) -
					(lblRightAnswer->Width / 2.));
				...
				imgChAnsLeft->Left = imgChoosenAnswer->Left -
					imgChAnsLeft->Width;
				imgChAnsRight->Left = imgChoosenAnswer->Left +
					imgChoosenAnswer->Width;
...
void __fastcall TF::FormShow(TObject* Sender) {
	...
	for (int i = 0; i < 5; ++i) {
		int j = i + 1;
		imgPlayer[i]->Top =
			int(imgHatch[j]->Top + (imgHatch[j]->Height - imgNumber[i]->Height)
			/ 2 - 5);
		imgPlayer[i]->Left =
			int(imgHatch[j]->Left + (imgHatch[j]->Width - imgNumber[i]->Width)
			/ 2 - 5);
	}
...
void __fastcall TF::FormResize(TObject* Sender) {
	...
	LabelQuestion->Top = imgQuestion->Top + 47;
	F->LabelQuestion->Left =
		(int)(F->imgQuestion->Left + (F->imgQuestion->Width -
		F->LabelQuestion->Width) / 2.);
	...
	int offcet = 0;
	for (int i = 0; i < 5; ++i) {
		CreateLabel(lblPlayer, i, 15 + offcet, imgPlayers->Left + 67, 201, 20,
			Settings->PlayerNames[i]);
		CreateLabel(lblMoney, i, 43 + offcet, imgPlayers->Left + 80, 176, 25,
			money[i]);
		offcet += 83;
	}
...
void ShowError(int errcode) {
	...
	case 2:
		F->FatalError = 1;
		ShowMessage("Ошибка загрузки базы вопросов: база с именем '" +
			Settings->LastBase + "' не существует!");
		exit(1);
	}
}
...
void DumpMemory(int errcode) {
	...
	str = "Question = " + IntToStr(base[n].Round) + ". " +
		base[n].Question + "\n\r";
	stream->Write(& str[1], str.Length());
...
void __fastcall TF::ControlLabelClick(TObject* Sender) {
	...
	if (ModeOfGame == 1 && CanChoose == 1 && Settings->PlayerType[CurrentHatch -
		1] == bbHuman) {
		for (int i = 0; i < 5; ++i) {
...
void TF::ResizeAnswers() {
	if (RoundOfGame == 1) {
		...
		lblAnswers[0]->Left =
			(int)(imgQuestion->Left + middle - 50 -
			(lblAnswers[0]->Width + lblAnswers[1]->Width) / 2.);
		lblAnswers[1]->Left =
			(int)(lblAnswers[0]->Left + (lblAnswers[0]->Width) + 100);
	}
	if (RoundOfGame == 2) {
		int middle = (int)((imgQuestion->Width) / 2.);
		if (lblAnswers[0]->Width > lblAnswers[2]->Width) {
			...
			lblAnswers[2]->Left =
				(int)(imgQuestion->Left + middle - 50 -
				(lblAnswers[2]->Width + lblAnswers[1]->Width) / 2.);
			lblAnswers[1]->Left =
				(int)(lblAnswers[2]->Left + (lblAnswers[2]->Width) + 100);
	...
	if (RoundOfGame == 4) {
		...
		lblAnswers[0]->Left =
			(int)(imgQuestion->Left + middle - 100 -
			(lblAnswers[0]->Width + lblAnswers[1]->Width +
			lblAnswers[2]->Width) / 2.);
		lblAnswers[1]->Left =
			(int)(lblAnswers[0]->Left + (lblAnswers[0]->Width) + 100);
		lblAnswers[2]->Left =
			(int)(lblAnswers[1]->Left + (lblAnswers[1]->Width) + 100);
		lblAnswers[3]->Left =
			(int)(imgQuestion->Left + middle - 50 -
			(lblAnswers[3]->Width + lblAnswers[4]->Width) / 2.);
		lblAnswers[4]->Left =
			(int)(lblAnswers[3]->Left + (lblAnswers[3]->Width) + 100);
...
void TF::ShowAnswers() {
	if (RoundOfGame == 1) {
		CreateLabel(lblAnswers, 0, imgQuestion->Top + 160,
			imgQuestion->Left + 300,
			(base[NumberOfQuestion].Answers[variants[0]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[0]]);
		CreateLabel(lblAnswers, 1, imgQuestion->Top + 160,
			imgQuestion->Left + 600,
			(base[NumberOfQuestion].Answers[variants[1]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[1]]);
	}
	if (RoundOfGame == 2) {
		CreateLabel(lblAnswers, 0, imgQuestion->Top + 150,
			imgQuestion->Left + 300,
			(base[NumberOfQuestion].Answers[variants[0]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[0]]);
		CreateLabel(lblAnswers, 1, imgQuestion->Top + 150,
			imgQuestion->Left + 600,
			(base[NumberOfQuestion].Answers[variants[1]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[1]]);
		CreateLabel(lblAnswers, 2, imgQuestion->Top + 175,
			imgQuestion->Left + 300,
	...
	if (RoundOfGame == 3) {
		...
		CreateLabel(lblAnswers, 2, imgQuestion->Top + 175,
			...
			(base[NumberOfQuestion].Answers[variants[2]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[2]]);
		CreateLabel(lblAnswers, 3, imgQuestion->Top + 175,
			imgQuestion->Left + 600,
	...
	if (RoundOfGame == 4) {
		CreateLabel(lblAnswers, 0, imgQuestion->Top + 150,
			imgQuestion->Left + 150,
			(base[NumberOfQuestion].Answers[variants[0]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[0]]);
		CreateLabel(lblAnswers, 1, imgQuestion->Top + 150,
			imgQuestion->Left + 450,
			(base[NumberOfQuestion].Answers[variants[1]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[1]]);
		CreateLabel(lblAnswers, 2, imgQuestion->Top + 150,
			imgQuestion->Left + 750,
			(base[NumberOfQuestion].Answers[variants[2]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[2]]);
		CreateLabel(lblAnswers, 3, imgQuestion->Top + 175,
			imgQuestion->Left + 300,
			(base[NumberOfQuestion].Answers[variants[3]]).Length()* 12, 20,
			base[NumberOfQuestion].Answers[variants[3]]);
		CreateLabel(lblAnswers, 4, imgQuestion->Top + 175,
			imgQuestion->Left + 600,
